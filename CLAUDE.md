...
- `040_auto_link_sole_boat.sql` — cross-section boat sync at the data layer (fixes jobs stuck on "No boat" after a boat is added to the client). Triggers: `boats_trim_fields` (btrim `name`/`make_model` on write), `boats_link_boatless_records` (adding a customer's FIRST boat auto-links their boat-less jobs + work_orders), `jobs_default_boat` / `work_orders_default_boat` (shared `default_boat_from_customer()`: new row with no boat defaults to the customer's sole boat; changing a job's customer clears a boat that doesn't belong to the new customer). Multi-boat customers are never auto-linked (ambiguous — human decides); paperwork blocks exempt. Includes one-time backfill. DB-level, so all three UIs **and already-shipped mobile builds** get the behavior with no app update. Verified end-to-end on the live dashboard (Playwright smoke test 2026-07-02).
- `041_boat_link_hardening.sql` — **⚠️ in repo, NOT YET APPLIED (prod apply awaiting Connor's approval).** Hermes cross-agent audit follow-ups on 040, all four findings verified: per-customer `pg_advisory_xact_lock` around sole-boat decisions (concurrent first-boat insert race), new `reconcile_customer_sole_boat()` shared by AFTER INSERT / AFTER UPDATE OF customer_id (`boats_owner_changed`, also detaches the moved boat from the old customer's records) / AFTER DELETE (`boats_deleted`) triggers, WO auto-link gated on `wo_can_edit()` for end-user paths (system paths with `auth.uid() is null` trusted), and house-style `REVOKE`s on all the trigger functions.
- `042_admin_customer_office.sql` — **⚠️ in repo, NOT YET APPLIED (prod apply awaiting Connor's approval).** Fixes the invisible-client bug found in the 2026-07-02 prod smoke test: `set_customer_tenant` (023) clobbered `customers.location_id` with the org-wide admin's NULL profile location, so a client created while office-filtered to Seattle vanished from every Seattle-filtered view. Org-wide callers now keep an explicitly chosen office (validated against their org); location-bound staff are still forced to their own office. Pairs with the Clients page passing `activeLocationId` on create (`app/dashboard/customers/{page,customer-list}.tsx` — needs a dashboard deploy once applied).
