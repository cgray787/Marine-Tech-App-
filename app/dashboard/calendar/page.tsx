...
import { statusStripeColor } from '@/lib/calendar/colors';
  ...
  return (
    <div className="p-6 text-white min-h-screen bg-[#060a12]">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-x-6 gap-y-2 text-xs text-[#8892A5]">
        <span className="text-[#C9A96E] uppercase tracking-wider">Bars:</span>
        <span>Colored per client · left stripe = status</span>
      ...
      {scheduleJob && (
        <ScheduleQuickPicker
          ...
          onSave={async ({ scheduledStart, scheduledEnd, scheduledEndDate, dayLocations }) => {
            await updateJob(supabase, {
              id: scheduleJob.id,
              scheduledStart,
              scheduledEnd,
              ...(scheduledEndDate != null ? { scheduledEndDate } : {}),
              ...(dayLocations != null ? { dayLocations } : {}),
            });
